...
const getSubjects = AsyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (!subjects?.length) {
        return res.status(200).json(new ApiResponse(true, "No subjects found", []));
    }

    res.json(new ApiResponse(true, 'Subjects fetched', subjects));
});
...
const addSubject = AsyncHandler(async(req:AuthenticatedRequest,res:Response)=>{
    ...
    if(!subName) throw new ApiError("Could not get subject name",402)

    const subExist = await Subject.findOne({
        subName 
    });
    ...
    if(subExist) 
        return res
        .status(400)
        .json(new ApiResponse(false,"Subject already exists",subExist))
    ...
    return res
    .status(200)
    .json(new ApiResponse(true,"New Subject Added",subject))
})
